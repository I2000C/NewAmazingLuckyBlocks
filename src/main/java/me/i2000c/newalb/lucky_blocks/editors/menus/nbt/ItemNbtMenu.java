...
public class ItemNbtMenu extends EditorMenu<ItemStack> {
	
    public ItemNbtMenu() {
		super("&3&lNBT menu", MenuSize.SIZE_4_ROWS, true);
	}
    ...
    protected void buildMenu(Player player) {
    	if(rootTag == null) {
    		rootTag = new CustomNBTItem(item.clone());
    		currentTag = rootTag;
    		editMode = false;
    		deleteMode = false;
    	}
        
        Set<String> keys = new TreeSet<>(currentTag.getKeys());
            
        Iterator<String> iterator = keys.iterator();
        for(int i=0; i<BACK_SLOT && iterator.hasNext(); i++) {
            String key = iterator.next();
            ItemStack stack = getTagItem(currentTag, key);
            setItem(i, stack, e -> {
            	NBT.get(stack, nbt -> {
                    ...
                    String nbtKey = nbt.getString(NBT_NAME_TAG);
                    
                    if(editMode) {
                        ItemNbtTagMenu.Tag tag = new ItemNbtTagMenu.Tag();
                        tag.setName(nbtKey);
                        tag.setType(nbtType);
                        tag.setValue(getValue(currentTag, nbtKey));
                        
                        ItemNbtTagMenu menu = new ItemNbtTagMenu();
                        menu.setItemToEdit(tag);
                        menu.setOnBack(this::openToPlayer);
                        menu.setOnNext((p, editedTag) -> {
                        	setTag(editedTag);
                        	openToPlayer(player);
                        });
                        menu.openToPlayer(player);
                    } else if(deleteMode) {
                        currentTag.removeKey(nbtKey);
                        openToPlayer(player);
                    } else {
                        if(nbtType == NBTType.NBTTagCompound) {
                            // Value is the NBT key
                            currentTag = currentTag.getCompound(nbtKey);
                            openToPlayer(player);
                        }
                    }
                });
        ...
        setItem(TREE_ITEM_SLOT, treeItem);
        ...
        if(Objects.equals(rootTag, currentTag)) {
            ...
            setItem(GO_TO_PARENT_TAG_SLOT, goToParentTagItem, e -> {
        ...
        if(!editMode && !deleteMode) {
            setItem(ADD_NBT_TAG_SLOT, add, e -> {
            	ItemNbtTagMenu menu = new ItemNbtTagMenu();
            	menu.setOnBack(this::openToPlayer);
            	menu.setOnNext((p, tag) -> {
            		setTag(tag);
            		openToPlayer(p);
            	});
            	menu.openToPlayer(player);
        ...
        if(!deleteMode) {
            setItem(EDIT_NBT_TAG_SLOT, edit, e -> {
            	editMode = !editMode;
        ...
        if(!editMode) {
            setItem(DELETE_NBT_TAG_SLOT, delete, e -> {
