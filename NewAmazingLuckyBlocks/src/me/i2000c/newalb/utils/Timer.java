...
public class Timer implements Listener{
    ...
    public void executeDarkHole(Player player, Location location, int height, double radius, long ticks, long before_ticks, boolean squared){
        ...
        Sound sound = XSound.WITHER_IDLE.playSound();
        Effect effect;
        /*try {
...
//</editor-fold>
    ...
    public void executeMiniVolcano(Player player, Location location, int height, Material baseMaterial, Material lavaMaterial, long ticks, long before_ticks, boolean squared, boolean throwBlocks){
        //<editor-fold defaultstate="collapsed" desc="Code">
        Sound sound = XSound.FUSE.playSound();
        Effect effect;
        /*try{
...
//</editor-fold>
    ...
    public void executeFrostPathWand(Player player, Material frostMaterial, int blocks, int maxBlocks,
        ...
        Set<Material> passableBlocks = EnumSet.of(Material.AIR, Material.WATER, Material.LAVA, Material.GRASS);
                ...
                if(i > maxBlocks || i > distance){
                    ...
                    do{
                        ...
                        if(passableBlocks.contains(location.getWorld().getBlockAt(location).getType())){
                            ...
                            BlockPlaceEvent e2 = new BlockPlaceEvent(b, b.getState(), b, new ItemStack(b.getType()), player, true);
                            Bukkit.getPluginManager().callEvent(e2);
                            if(!e2.isCancelled()){                                
                                b.setType(frostMaterial);
                                //player.getWorld().playEffect(location.clone().add(0,1,0), Effect.SNOWBALL_BREAK, 0);
                                player.getWorld().playSound(location, XSound.GLASS.playSound(), 2.0F, 1.0F);
                            }  
                        }
            ...
            public void run(){
                ...
                if(i>=2){
                    ...
                    if(!protect || (protect && b.getType().equals(Material.AIR))){
                        ent.getWorld().playSound(l, XSound.GLASS.playSound(), 2.0F, 1.0F);
